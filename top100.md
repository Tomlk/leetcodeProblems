    ...
    public boolean isValid(String s) {
        ...
        for(int i=1;i<s.length();i++){
            ...
            if(c.equals('(') || c.equals('{') ||c.equals('[')){
                ...
                if(stack.size()==0)
                    return false;
                Character peekC=stack.getLast();
                if(c.equals(')') &&peekC.equals('('))
                    stack.removeLast();
                else if(c.equals('}') &&peekC.equals('{'))
                     stack.removeLast();
                else if(c.equals(']') &&peekC.equals('['))
                    stack.removeLast();
                else 
...
class Solution {
    public List<Integer> partitionLabels(String s) {
        ...
        while(i<n){
            int start=i;
            maxIndex=charLastIndex.get(s.charAt(start));
            while(i<=maxIndex){
                maxIndex=Math.max(maxIndex,charLastIndex.get(s.charAt(i)));
                i+=1;
            }
            result.add(i-start);
        }
